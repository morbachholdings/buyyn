...
class Submissions extends CI_Controller
{
	...
	public function submissions()
	{
		if ($this->session->userdata('full_name') != '') {
			...
			$data["view_invoiced"]=$this->msubmissions->view_invoiced_submission_table();
			$data["pending_years"]=$this->msubmissions->view_invoiced_submission_table();
	...
	public function insert_child_invoices()
	{
		if ($this->session->userdata('full_name') != '') {
			$email = $this->session->userdata('full_name');
			$user_id = $this->musers->user_id($email);
			$member_name = $this->input->post('members_name');
	...
	public function view_submission()
	{
		if ($this->session->userdata('full_name') != '') {
			...
			// var_dump($push["supplier_data"]->result());
			// die();
			$this->load->view('header', $value);
	...
	public function submission_deleted()
	{
		if ($this->session->userdata('full_name') != '') {
			$email= $this->session->userdata('full_name');
			$data["updated_by"] = $this->musers->user_id($email);
