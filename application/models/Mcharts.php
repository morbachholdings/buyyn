...
ORDER BY month_id";
    ...
    public function member_data($data)
    {
        $sql = "SELECT t1.month_name, CASE WHEN SUM(t2.amount) > 0 THEN SUM(t2.amount) ELSE 0 END AS Total FROM
        (
            SELECT months.month_name, month_id
        ...
        LEFT JOIN ( 
            SELECT submissions.financial_month, submissions.amount
            FROM submissions
            WHERE submissions.Supplier_ID=".$data." AND submissions.is_child = 1 
        ) t2 ON t1.month_name = t2.financial_month
        GROUP BY month_name
        ORDER BY month_id";
         return $this->db->query($sql);
    ...
    public function member_data_single($val)
    {
        $sql="SELECT t1.month_name, CASE WHEN SUM(t2.amount) > 0 THEN SUM(t2.amount) ELSE 0 END AS Total FROM
        (
            SELECT months.month_name, month_id
        ...
        LEFT JOIN ( 
            SELECT submissions.financial_month, submissions.amount
            FROM submissions
            WHERE submissions.financial_year='".$val["year"]."' AND submissions.Supplier_ID=".$val["user_id"]." AND Member_ID=".$val["member_ID"]." AND submissions.is_child = 1 
        ) t2 ON t1.month_name = t2.financial_month
        GROUP BY month_name
        ORDER BY month_id";
         return $this->db->query($sql);
    ...
    public function member_data_single_year($val)
    {
        $sql="SELECT t1.month_name, CASE WHEN SUM(t2.amount) > 0 THEN SUM(t2.amount) ELSE 0 END AS Total FROM
        (
            SELECT months.month_name, month_id
        ...
        LEFT JOIN ( 
            SELECT submissions.financial_month, submissions.amount
            FROM submissions
            WHERE submissions.financial_year= '".$val["year"]."' AND submissions.Supplier_ID=".$val["user_id"]." AND submissions.is_child = 1 
        ) t2 ON t1.month_name = t2.financial_month
        GROUP BY month_name
        ORDER BY month_id";
         return $this->db->query($sql);
